...
/**
 * COMPREHENSIVE TRANSITION DEBUGGER
 * 
 * This class identifies and fixes all common transition/rendering issues:
 * 1. Memory management problems
 * 2. Frame flushing issues
 * 3. Threading problems
 * 4. Mat → Frame conversion issues
 * 5. Video recorder synchronization
 */
public class TransitionDebugger {
    ...
    public static void main(String[] args) {
        ...
        TransitionDebugger debugger = new TransitionDebugger();
    ...
    public void testMatOperations() {
        ...
        try {
            ...
            System.out.println("✅ Created test frames: " + frame1.cols() + "x" + frame1.rows());
            ...
            if (!blended.empty() && blended.cols() > 0 && blended.rows() > 0) {
                System.out.println("✅ Blending successful: " + blended.cols() + "x" + blended.rows());
                ...
                System.err.println("❌ Blended frame is invalid!");
            }
            ...
            System.err.println("❌ Mat operations test failed: " + e.getMessage());
        }
    ...
    public void testFrameConversion() {
        ...
        try {
            ...
            if (testMat.empty()) {
                System.err.println("❌ Test Mat is empty!");
                return;
            }
            
            System.out.println("✅ Test Mat created: " + testMat.cols() + "x" + testMat.rows() + 
            ...
            if (frame == null || frame.image == null) {
                System.err.println("❌ Frame conversion failed!");
                return;
            }
            
            System.out.println("✅ Mat → Frame conversion successful");
            ...
            if (convertedBack.empty()) {
                System.err.println("❌ Frame → Mat conversion failed!");
                return;
            }
            
            System.out.println("✅ Frame → Mat conversion successful");
            ...
            System.out.println("✅ Frame conversion test completed");
            
        } catch (Exception e) {
            System.err.println("❌ Frame conversion test failed: " + e.getMessage());
            e.printStackTrace();
        }
    ...
    public void testVideoRecorder() {
        ...
        try {
            ...
            for (int i = 0; i < 60; i++) { // 2 seconds at 30fps
                ...
                if (frame.empty()) {
                    System.err.println("❌ Frame " + i + " is empty!");
                    continue;
                }
                ...
                if (videoFrame == null || videoFrame.image == null) {
                    System.err.println("❌ Frame " + i + " conversion failed!");
                    frame.release();
                    continue;
                }
                ...
                if (i % 10 == 0) {
                    recorder.flush();
                    System.out.println("✅ Flushed at frame " + i);
                }
            ...
            if (outputFileObj.exists() && outputFileObj.length() > 1000) {
                System.out.println("✅ Video recorded successfully: " + outputFile);
                System.out.println("   File size: " + outputFileObj.length() + " bytes");
            } else {
                System.err.println("❌ Video recording failed or file too small");
            }
            
        } catch (Exception e) {
            System.err.println("❌ Video recorder test failed: " + e.getMessage());
            e.printStackTrace();
        } finally {
            if (recorder != null) {
                try {
    ...
    public void testTransitionRendering() {
        ...
        try {
            ...
            for (TransitionType transitionType : transitions) {
                ...
                try {
                    ...
                    for (int i = 0; i < progressPoints.length; i++) {
                        ...
                        if (result.empty()) {
                            System.err.println("❌ Transition result is empty at progress " + progress);
                            continue;
                        }

                        // Save debug frame
                        String filename = OUTPUT_DIR + "/test4_" + transitionType.toString().toLowerCase() +
                                        "_progress_" + (int)(progress * 100) + ".jpg";
                        imwrite(filename, result);

                        result.release();
                    }

                    System.out.println("✅ " + transitionType + " transition test completed");

                } catch (Exception e) {
                    System.err.println("❌ " + transitionType + " transition failed: " + e.getMessage());
                }
            }
            ...
            System.err.println("❌ Transition rendering test failed: " + e.getMessage());
        }
    ...
    /**
     * Test 5: Async processing to avoid blocking
     */
    public void testAsyncProcessing() {
        System.out.println("🧪 Test 5: Async Processing (Non-blocking)");

        try {
            // Simulate async transition processing
            CompletableFuture<Void> future = CompletableFuture.runAsync(() -> {
                try {
                    System.out.println("🔄 Starting async transition processing...");

                    Mat frame1 = createTestFrame(new Scalar(255, 100, 100, 255), "ASYNC 1");
                    Mat frame2 = createTestFrame(new Scalar(100, 255, 100, 255), "ASYNC 2");

                    // Simulate processing time
                    Thread.sleep(1000);

                    // Create transition
                    BaseTransition transition = new FadeTransition(WIDTH, HEIGHT, 30, TransitionType.CROSSFADE);
                    Mat result = transition.applyTransition(frame1, frame2, 0.5);

                    // Save result
                    imwrite(OUTPUT_DIR + "/test5_async_result.jpg", result);
            ...
            System.err.println("❌ Async processing test failed: " + e.getMessage());
        }
    ...
    /**
     * Create transition instance for testing
     */
    private BaseTransition createTransition(TransitionType type) {
        switch (type) {
            case CROSSFADE:
                return new FadeTransition(WIDTH, HEIGHT, 30, type);
            case SLIDE_LEFT:
                return new SlideTransition(WIDTH, HEIGHT, 30, type);
            case ZOOM_IN:
                return new ZoomTransition(WIDTH, HEIGHT, 30, type);
            default:
                return new FadeTransition(WIDTH, HEIGHT, 30, TransitionType.CROSSFADE);
        }
    }
